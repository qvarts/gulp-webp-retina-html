...
const normaliseEOL = (str) => str.toString().replace(/\r\n/g, '\n');
...
describe('Tests', () => {
  ...
  it('should compile html file with webp and retina image support for all supported image types', (done) => {
    ...
    const stream = webpHtml({
      extensions: [
        'jpg', 'jpeg', 'jfif', 'pjpeg', 'pjp', 'png', 'apng', 'gif', 'svg', 'webp',
  ...
  it(
    'should compile html file with webp and retina image support for all supported image types without 1x retina suffix',
    (done) => {
      const htmlFile = createVinyl('check_all.html');
      const stream = webpHtml({
        extensions: [
      ...
      stream.on('data', (updatedFile) => {
        ...
        assert.ok(updatedFile.contents);
        assert.equal(path.basename(updatedFile.path), 'check_all.html');
        const actual = fs.readFileSync(path.join(__dirname, expectedTestsPath, 'check_all_without_retina_1x.html'),
          'utf8');
        assert.equal(String(normaliseEOL(updatedFile.contents)), normaliseEOL(actual));
        done();
      });
  ...
  it('should compile html file with correct lazy load data attributes', (done) => {
    const htmlFile = createVinyl('lazy.html');
    const stream = webpHtml({
      retina: {
        1: '',
        2: '@2x',
    ...
    stream.on('data', (updatedFile) => {
      ...
      assert.ok(updatedFile.relative);
      assert.ok(updatedFile.contents);
      assert.equal(path.basename(updatedFile.path), 'lazy.html');
      const actual = fs.readFileSync(path.join(__dirname, expectedTestsPath, 'lazy.html'), 'utf8');
      assert.equal(String(normaliseEOL(updatedFile.contents)), normaliseEOL(actual));
      done();
    });
    stream.write(htmlFile);
